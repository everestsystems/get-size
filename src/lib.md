...
```rust
use get_size::{GetSize, NoTracker};

fn main() {
  let value = String::from("Hello World!");

  assert_eq!(String::get_stack_size(), std::mem::size_of::<String>());
  assert_eq!(value.get_heap_size(&mut NoTracker), 12);

  assert_eq!(value.get_size(&mut NoTracker), std::mem::size_of::<String>() + 12);
  ...
  assert_eq!(buffer.get_heap_size(&mut NoTracker), 1024);
}
```
...
fn main() {
  ...
  assert_eq!(value.get_heap_size(&mut NoTracker), 5);
  assert_eq!(GetSize::get_heap_size(&&value, &mut NoTracker), 0); // Fully qualified syntax
  ...
  // accounted for.
  assert_eq!(value.get_heap_size(&mut NoTracker), 0);
}
```
...
fn main() {
  ...
  assert_eq!(value.get_heap_size(&mut NoTracker), std::mem::size_of::<String>() + 5);
}
```
...
fn main() {
  ...
  assert_eq!(primary_data.get_heap_size(&mut NoTracker), Vec::<u8>::get_stack_size() + 1024);
  assert_eq!(secondary_data.get_heap_size(&mut NoTracker), 0);
}
```
...
fn main() {
  ...
  assert_eq!(test.get_heap_size(&mut NoTracker), 1024);
}
```
...
fn main() {
  ...
  assert_eq!(test.get_heap_size(&mut NoTracker), 512);
}
```
